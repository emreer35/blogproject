...
<body>
    @if (session('success'))
        <div
            class="success-message absolute z-30 top-20 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white dark:bg-white dark:text-slate-900 p-3 rounded-sm shadow-md opacity-100 translate-y-[-0%] transition-all duration-500 w-96 text-sm ">
            {{ session('success') }}
        </div>
    @endif
    @if (session('error'))
        <div
            class=" absolute z-30 top-20 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white dark:bg-white dark:text-slate-900 p-3 rounded-sm shadow-md opacity-1000 translate-y-[-0%] transition-all duration-500 w-96 text-sm ">
            {{ session('error') }}
        </div>
    @endif
    <div class="font-[sans-serif] bg-gray-900 md:h-screen">
        <div class="grid md:grid-cols-2 items-center gap-8 h-full">
            <div class="max-md:order-1 p-4">
                <img src="{{ asset('assets/image/darkmode.png') }}"
            ...
            <div class="flex items-center md:p-8 p-6 bg-white md:rounded-tl-[55px] md:rounded-bl-[55px] h-full">
                <form class="max-w-lg w-full mx-auto" action="{{ route('login') }}" method="POST">
                    @csrf
                    <div class="mb-12">
                        <h3 class="text-gray-800 text-4xl font-extrabold">{{ __('Giriş Yap') }}</h3>
                        <p class="text-gray-800 text-sm mt-4 ">{{ __('Hesabın yok mu?') }}<a
                                href="{{ route('register') }}"
                                class="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">{{ __('Kayıt Ol') }}</a>
                        </p>
                    </div>
                    ...
                    <div class="mt-8">
                        <label class="text-gray-800 text-xs block mb-2">Şifre</label>
                        <div class="relative flex items-center">
                            <input name="password" type="password" required id="password"
                                class="w-full text-sm border-b border-gray-300 focus:border-gray-800 px-2 py-3 outline-none"
                                placeholder="Şifre" />

                            <svg id="showPassword" xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#ccc"
                                class="w-[18px] h-[18px] absolute right-2 cursor-pointer" viewBox="0 0 24 24"
                            ...
                            <svg id="hidePassword" xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#bbb"
                                class="w-[18px] h-[18px] absolute right-2 cursor-pointer hidden" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
