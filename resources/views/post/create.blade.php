...
@section('content')
    <div class="col-span-3">
        <h1 class="text-lg font-medium lg:text-2xl mb-4 text-slate-900 dark:text-white">{{ __('Yeni Post Oluştur') }}</h1>
        <hr class="mb-4">
            ...
            <form action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex items-center justify-center w-full mb-4">
                    <label for="dropzone-file"
                        class="flex flex-col items-center justify-center w-full lg:w-3/4 p-4 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                        <img id="uploadedImage" class="w-full h-56 lg:h-72 object-cover rounded hidden"
                            alt="Uploaded Image">
                        <!-- Varsayılan yazı ve ikon -->
                        <div id="uploadArea" class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p id="uploadMessage" class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                                Yüklemek için <span class="font-semibold">sürükleyin ya da tıklayın</span>
                            </p>
                            <p id="againuploadMessage" class="hidden  text-sm text-gray-500 dark:text-gray-400">
                ...
                <div class="mb-4">
                    <input
                        ...
                        type="text" name="title" id="title" placeholder="Başlık">
                    @error('title')
                        <div class="mt-1 text-xs lg:text-sm text-red-500 dark:text-red-400 ">{{ $message }}</div>
                    @enderror
    ...
    <script>
        ...
        fileInput.addEventListener('change', (event) => {
            ...
            if (file) {
                ...
                reader.onload = (e) => {
                    // Resmi yükleyip göster
                    uploadedImage.src = e.target.result;
                    uploadedImage.classList.remove('hidden');

                    // İlk yükleme mesajını gizle, değiştirme mesajını göster
                    uploadMessage.classList.add('hidden');
                    againuploadMessage.classList.remove('hidden');
                };

                reader.readAsDataURL(file); // Resmi okuyarak base64'e çevir
            }
        });
        ...
        textarea.addEventListener('input', () => {
            // Otomatik boyutlandırmayı sağlamak için önce yüksekliği sıfırla
